...
class Factor_3_2_3_3_Analyzer_Fixed:
    ...
    def _can_patient_be_pregnant(self, patient_data: Dict[str, Any]) -> bool:
        """Determine if patient could potentially be pregnant"""
        ...
        # Females over 50 (unless explicitly pregnant)
        ...
        for pattern in patterns:
            ...
            for match in matches:
                ...
                for potential_adr in potential_adrs:
                    ...
                    adr_name = self._clean_serious_adr_name(adr_clean)
    ...
    def _extract_adr_name(self, context: str, keyword: str) -> Optional[str]:
        """Extract the actual ADR name from context"""
        
        context_lower = context.lower()
        
        # Priority 1: Check for specific serious medical conditions
        serious_conditions = [
        ...
        patterns = [
            r'([a-z\-]+)-associated\s+([a-z\s]+?)(?:\s+have|\s+has|\s+may|\s+can|\.)',
            r'cases of\s+([a-z\s]+?)(?:\s+have|\s+has|\s+may|\.|\s+in)',
            r'risk of\s+([a-z\s]+?)(?:\s+in|\s+and|\.|,)',
            r'including\s+(?:fatal\s+)?(?:and\s+)?(?:non-fatal\s+)?([a-z\s]+?)(?:\.|,|\s+and)',
        ...
        for pattern in patterns:
            ...
            if match:
                groups = match.groups()
                adr_name = groups[-1].strip() if groups else None
                
                if adr_name and len(adr_name) > 5 and len(adr_name) < 60:
                    # Skip if it's an invalid ADR
                    invalid_terms = ['death', 'fatal', 'mortality', 'life-threatening',
                                   'can be fatal', 'hepatoxicity', 'serious']
                    if adr_name not in invalid_terms:
                        return adr_name.title()
        
        # Priority 3: If keyword is already a condition
        if keyword.lower() in [c.lower() for c in serious_conditions]:
            return keyword.title()
        
        # Skip generic outcomes
        exclude_outcomes = ['death', 'fatal', 'mortality', 'life-threatening', 
                          'can be fatal', 'hospitalization']
        if keyword.lower() in exclude_outcomes:
            return None
...
def main():
    ...
    # Run analysis
    results = analyzer.analyze(patient_input)
    
    # Print report
    analyzer.print_report(results)
    
    # Save results
    output_file = f"factor_3_2_3_3_report_CORRECTED_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
